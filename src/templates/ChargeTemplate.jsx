...
import { getChargeContent } from '../api/contentApi';
...
export default function ChargeTemplate() {
  ...
  const handlers = useSwipeable({
    onSwipedLeft: () => navigate('/whosfan'),
    onSwipedRight: () => navigate('/store'),
    preventScrollOnSwipe: true,
    trackMouse: true,
  });

  const getContent = async (num) => {
    const { content } = await getChargeContent(num);
  ...
  return (
    <div {...handlers}>
      <CurationTitle>충전소</CurationTitle>
